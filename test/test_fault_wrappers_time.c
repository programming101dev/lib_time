...
static void write_outcome(const char *wrapper, const char *domain, const char *symbol, int code, int passed)
{
    ...
    {
        written = fprintf(outcome_stream, "P101WRAPPER\t1\tFAULT\t%s\tlib_time\t%s\t%s\t%s\t%d\t%s\n", P101_TEST_PLATFORM, wrapper, domain, symbol, code, passed ? "PASS" : "FAIL");
...
static void test_p101_clock_getres(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        write_outcome("p101_clock_getres", "errno", error_names[index], state.code, failures == failures_before);
    ...
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            int             native_result     = p101_clock_getres(native_env, native_err, 0, &native_argument_3);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                ...
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_clock_getres: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
...
static void test_p101_clock_gettime(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        EXPECT(result == (-1));
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_clock_gettime", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            struct timespec native_argument_3 = {0};
            int             native_result     = p101_clock_gettime(native_env, native_err, 0, &native_argument_3);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                bool native_error_declared = false;
                ...
                if(!native_error_declared)
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_clock_gettime: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
                p101_error_reset(native_err);
            }
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_clock_gettime: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status) && WEXITSTATUS(native_status) == 77)
            {
                fprintf(stderr, "native smoke fixture unavailable: p101_clock_gettime\n");
            }
            else if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_clock_gettime: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_clock_settime(struct p101_env *env, struct p101_error *err)
{
    ...
    static const char *const error_names[] = {"EACCES", "EFAULT", "EINVAL", "ENODEV", "ENOTSUP", "EOVERFLOW", "EPERM"};
#elif defined(__APPLE__)
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        EXPECT(result == (-1));
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_clock_settime", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            struct timespec native_argument_3 = {0};
            int             native_result     = p101_clock_settime(native_env, native_err, 0, &native_argument_3);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                bool native_error_declared = false;
                ...
                if(!native_error_declared)
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_clock_settime: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
                p101_error_reset(native_err);
            }
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_clock_settime: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status) && WEXITSTATUS(native_status) == 77)
            {
                fprintf(stderr, "native smoke fixture unavailable: p101_clock_settime\n");
            }
            else if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_clock_settime: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_gmtime_r(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        EXPECT(result == (NULL));
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_gmtime_r", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            time_t     native_argument_2 = {0};
            struct tm  native_argument_3 = {0};
            struct tm *native_result     = p101_gmtime_r(native_env, native_err, &native_argument_2, &native_argument_3);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                bool native_error_declared = false;
                ...
                if(!native_error_declared)
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_gmtime_r: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
                p101_error_reset(native_err);
            }
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_gmtime_r: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status) && WEXITSTATUS(native_status) == 77)
            {
                fprintf(stderr, "native smoke fixture unavailable: p101_gmtime_r\n");
            }
            else if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_gmtime_r: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_localtime_r(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        EXPECT(result == (NULL));
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_localtime_r", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            time_t     native_argument_2 = {0};
            struct tm  native_argument_3 = {0};
            struct tm *native_result     = p101_localtime_r(native_env, native_err, &native_argument_2, &native_argument_3);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                bool native_error_declared = false;
                ...
                if(!native_error_declared)
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_localtime_r: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
                p101_error_reset(native_err);
            }
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_localtime_r: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status) && WEXITSTATUS(native_status) == 77)
            {
                fprintf(stderr, "native smoke fixture unavailable: p101_localtime_r\n");
            }
            else if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_localtime_r: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_nanosleep(struct p101_env *env, struct p101_error *err)
{
    ...
    static const char *const error_names[] = {"EFAULT", "EINTR", "EINVAL", "ENOTSUP"};
#else
    static const int         errors[]      = {EINTR, EINVAL};
    static const char *const error_names[] = {"EINTR", "EINVAL"};
#endif
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        EXPECT(result == (-1));
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_nanosleep", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            struct timespec native_argument_2 = {0};
            struct timespec native_argument_3 = {0};
            int             native_result     = p101_nanosleep(native_env, native_err, &native_argument_2, &native_argument_3);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                bool native_error_declared = false;
                ...
                if(!native_error_declared)
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_nanosleep: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
                p101_error_reset(native_err);
            }
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_nanosleep: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status) && WEXITSTATUS(native_status) == 77)
            {
                fprintf(stderr, "native smoke fixture unavailable: p101_nanosleep\n");
            }
            else if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_nanosleep: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_strftime_l(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        errno                 = P101_TEST_ERRNO_SENTINEL;
        p101_env_set_fault_injector(env, fail_next_call, &state);
        size_t result = p101_strftime_l(env, err, argument_2, 0, NULL, NULL, (locale_t){0});
        (void)result;
        EXPECT(state.checks == 1);
        EXPECT(p101_error_is_errno(err, state.code));
        EXPECT(errno == P101_TEST_ERRNO_SENTINEL);
    ...
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            locale_t  native_argument_6;
            native_argument_6 = newlocale(LC_ALL_MASK, "C", (locale_t)0);
            if(native_argument_6 == (locale_t)0)
            {
                native_child_status = 77;
                goto native_child_done_;
            }
            size_t native_result = p101_strftime_l(native_env, native_err, native_argument_2, 0, "p101", &native_argument_5, native_argument_6);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                bool native_error_declared = false;
                ...
                if(!native_error_declared)
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_strftime_l: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
                p101_error_reset(native_err);
            }
            freelocale(native_argument_6);
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        native_child_done_:
            p101_env_destroy(native_env);
            p101_error_destroy(native_err);
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_strftime_l: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status) && WEXITSTATUS(native_status) == 77)
            {
                fprintf(stderr, "native smoke fixture unavailable: p101_strftime_l\n");
            }
            else if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_strftime_l: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
